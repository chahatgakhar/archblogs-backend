...
// regex for password
let passwordRegex = /^(?=.*\d)(?=.*[-+_!@#$%^&*., ?])(?=.*[a-z])(?=.*[A-Z]).{6,12}$/;
...
const verifyJWT = (req, res, next) => {
    ...
    if(token == null){
        return res.status(401).json({ error: "There is no access token"})
    }

    jwt.verify(token, process.env.ACCESS_TOKEN, (err, user) => {
        if(err){
            return res.status(403).json({ error: "Your access token is invalid"});
        }
...
const getUsername = async (email) => {
    ...
    let nameNotUnique = await User
        .exists( { "personal_info.username": username })
        .then((result) => result)

        nameNotUnique ? username += nanoid().substring(0,4) : "";
        return username;
...
server.post("/signup", (req,res) => {
    ...
    if(fullname.length < 2){
        return res.status(403).json({ "error" : "Your Fullname must be at least 2 letters long"})
    }
    ...
    if(!passwordRegex.test(password)){
        return res.status(403).json({ "error" : "Please enter a valid Password containing 6-20 Character, number, uppercase chars, lowercase chars, a special character"})
    }

    bcrypt.hash(password, 8, async ( err, hash_pwd) => {

        let username = await getUsername(email);

        let user = new User({
            personal_info: { fullname, email, password: hash_pwd, username}
        });
        ...
        user
            ...
            .catch(err => {
                if( err.code == 11000) {
                    return res.status(500).json({ "error": "This Email address is already in use."})
                }
                return res.status(500).json({"error": err.message})
            })
...
server.post("/signin", (req,res) => {
   ...
   User.findOne({ "personal_info.email": email})
    .then((user) => {
        ...
        if(!user){
            return res.status(403).json({ "error" : "Email address not found. Please Sign Up!"});
        }
        ...
        if(!user.google_auth){
            ...
            return res.status(403).json({"error": "Account was created using google. Try logging in using Google. "})
        }
...
server.post("/google-auth", async (req, res) => {
    ...
    getAuth()
        ...
        .then( async (decodedUser) => {
            ...
            //when a person is already signed up
            if(user){
                if(!user.google_auth){
                    return res.status(403).json({"error": "This email is signed up using Google. Please log in with password associated to access the account"})
                }
            }
            //when a person has to signup
            else {
                let username= await getUsername(email);
                user= new User({
        ...
        .then( async (decodedUser) => {
            ...
            if(user){
                if(!user.facebook_auth){
                    return res.status(403).json({"error": "This email is signed up using Facebook. Please log in with password associated to access the account"})
                }
            }
            //when a person has to signup
            else {
...
server.post("/change-password", verifyJWT, (req,res) => {
    ...
    if (
      ...
      .then((user) => {
        ...
        if(user.google_auth){
            return res.status(403).json({error: "You can't change password of your account as its logged in through Google"})
        }
...
server.post("/update-profile", verifyJWT, (req, res)=>{
    ...
    if(username.length < 3) {
        return res.status(403).json({error: "Username should be greater than 3 letters"})
    }
        ...
        for(let i=0; i<socialLinksArray.length; i++){
            if(social_links[socialLinksArray[i]].length){
                ...
                if(!hostname.includes(`${socialLinksArray[i]}.com`)&& socialLinksArray[i] !== "website"){
                    return res.status(403).json({ error: `${socialLinksArray[i]} link is MongoInvalidArgumentError. You must enter a full link`})
                }
            }
        }
...
server.post("/create", verifyJWT, (req,res) => {
    
    let authID = req.user;
    let { title, des, banner, tags, content, draft, id } = req.body;

    if(!title.length){
        return res.status(403).json({ error: "Please provide a title to your blog"});
    }

    if(!draft){    
        if(!des.length || des.length >200 ){
            return res.status(403).json({error: "Please provide a description under 200 chars to publish your blog"});
        }
    
        if(!banner.length){
            return res.status(403).json({ error: "Please provide a blog banner to publish your blog"});
        }
    
        if(!content.blocks.length){
            return res.status(403).json({error: "Please provide some blog content to publish your blog"});
        }
    
        if(!tags.length || tags.length > 10){
            return res.status(403).json({error: "Please provide Tags to publish your blog"});
        }
    ...
    if(id){
        ...
        let blog = new Blog ({
            title, des, banner, content, tags, author: authID, blog_id, draft: Boolean(draft)
        })
        ...
        blog.save().then( blog => {
            ...
            User.findOneAndUpdate({ _id: authID}, { $inc: {"account_info.total_posts" : incrementVal }, $push : {"blogs": blog._id} }).then(user =>{
...
server.post("/search-user",(req, res) => {
    ...
    User.find({"personal_info.username": new RegExp(query, 'i')}).limit(50).select("personal_info.fullname personal_info.username personal_info.profile_img -_id ").then(users => {
...
server.post("/add-comment", verifyJWT, (req, res) => {
    ...
    new Comment(commentObject).save().then( async commentFile => {
        ...
        new Notification(notificationObject).save().then(notifications => console.log('new notification created'));
...
server.post("/notifications", verifyJWT, (req, res) => {
    ...
    .then(notifications => {
        ...
        .limit(maxLimit)
        .then(()=>{
            console.log("Notifications have been viewed")
        })
